  ...
  const popupContent = `
                <div class="p-2">
                    ...
                    <div class="space-y-1 text-sm">
                        ...
                        <p><strong>Kondisi:</strong> <span class="px-2 py-1 rounded text-xs ${
                          kondisi === "Baik"
      ...
      waitForElement("streetList", () => {
        ...
        if (streetList) {
          streetList.innerHTML =
            '<div class="p-4 text-center text-red-500">Error memuat data jalan</div>';
        }
      });
    });
}
...
function displayStreetList(data) {
  ...
  if (data && data.length > 0) {
    ...
    data.forEach((street) => {
      html += `
                        <div class="street-item p-4 hover:cursor-pointer hover:bg-gray-50 transition-colors duration-200" 
                             onclick="showMarkerOnMap(${street.latitude}, ${
        street.longitude
      }, '${street.nama_jalan}', '${street.alamat}', '${street.arah}', '${
        street.kondisi
      }', '${street.keterangan}')">
    ...
    streetList.innerHTML =
      '<div class="p-4 text-center text-gray-500">Tidak ada data jalan ditemukan</div>';
...
function waitForElement(elementId, callback, maxAttempts = 50) {
  ...
  const checkElement = () => {
    ...
    if (attempts < maxAttempts) {
      ...
      console.error(
        `Element dengan ID ${elementId} tidak ditemukan setelah ${maxAttempts} percobaan`
      );
    }
  };
